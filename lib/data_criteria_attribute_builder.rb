...
class DataCriteriaAttributeBuilder
  def build_data_criteria_for_measure(measure, valuesets)
    @valuesets = valuesets
    @measure = measure
    @vs_hash = {}
    @cql_data_criteria = []
  ...
  def update_measure_data_criteria
    @cql_data_criteria.each do |cdc|
      ...
      @measure.source_data_criteria.each do |sdc|
        next unless (valuesets.include? sdc.codeListId) || ((sdc.codeListId[0, 3] == 'drc') && (valuesets.include? @valuesets.select { |vs| vs['oid'] == sdc.codeListId }.first&.concepts&.first&.code))
        ...
        sdc['dataElementAttributes'] ? sdc['dataElementAttributes'] << dea : sdc['dataElementAttributes'] = [dea]
      end
    end
  end
